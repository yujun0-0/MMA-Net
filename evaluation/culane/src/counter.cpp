...
void Counter::setIOU(double value)
{
    iou = value;
}


// tp = 0, fp = 0, tn = 0, fn = 0, iou = 0
tuple<vector<int>, long, long, long, long, double> Counter::count_im_pair(const vector<vector<Point2f> > &anno_lanes, const vector<vector<Point2f> > &detect_lanes)
{
	...
	if(anno_lanes.empty() && detect_lanes.empty())
	{
		return make_tuple(anno_match, 0, detect_lanes.size(), 0, 0, 1);
	}
	if(anno_lanes.empty() && !detect_lanes.empty())
	{
	    return make_tuple(anno_match, 0, detect_lanes.size(), 0, 0, 0);
	}

	if(detect_lanes.empty() && !anno_lanes.empty())
	{
	...
	for(int i=0; i<anno_lanes.size(); i++)
	{
	    if(anno_match[i] >= 0)
	    {
	        ...
	        curr_iou +=  similarity[i][anno_match[i]];
//	        cout<< "similarity: " << similarity[i][anno_match[i]] << endl;
//	        cout<< "similarity: " << similarity[i][anno_match[i]] << endl;
...
//	cout << "curr_iou:" << curr_iou << "detect_lanes_size: "<< detect_lanes.size() << endl;
	double curr_miou = curr_iou / detect_lanes.size();
	return make_tuple(anno_match, curr_tp, curr_fp, 0, curr_fn, curr_miou);
}
